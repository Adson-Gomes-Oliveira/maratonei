...
async function fetchSerieDetail(showId) {
  try {
    const requestMovieDetails = await fetch(`https://api.themoviedb.org/3/tv/${showId}?api_key=${TMDB_API_KEY}&language=pt-BR`)
        .then((response) => response.json());

    const requestTrailerVideo = await fetch(`https://api.themoviedb.org/3/tv/${showId}/videos?api_key=${TMDB_API_KEY}&language=pt-BR`)
        .then((response) => response.json())
        .then((response) => response.results[0]);

    const requestRecomendations = await fetch(`https://api.themoviedb.org/3/tv/${showId}/recommendations?api_key=${TMDB_API_KEY}&language=pt-BR&page=1`)
        .then((response) => response.json())
        .then((response) => response.results);

    const requestProviders = await fetch(`https://api.themoviedb.org/3/tv/${showId}/watch/providers?api_key=${TMDB_API_KEY}`)
        .then((response) => response.json())
        .then((response) => response.results.BR);

    const requestReviews = await fetch(`https://api.themoviedb.org/3/tv/${showId}/reviews?api_key=${TMDB_API_KEY}&language=pt-BR&page=1`)
        .then((response) => response.json())
        .then((response) => response.results);

    const requestCastAndCrew = await fetch(`https://api.themoviedb.org/3/tv/${showId}/credits?api_key=${TMDB_API_KEY}&language=pt-BR`)
        .then((response) => response.json());
