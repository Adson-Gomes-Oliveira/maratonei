...
async function fetchMoviesByQuery(search, year) {
  ...
  const requestByQuery = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&query=${search}&language=pt-BR&year=${year}&include_adult=false&include_video=false&page=1&sort_by=popularity.desc&with_watch_monetization_types=flatrate`)
